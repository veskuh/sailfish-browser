...
void tst_declarativebookmarkmodel::initTestCase()
{
    ...
    QVERIFY(bookmarkModel->rowCount() > 0);
}
...
void tst_declarativebookmarkmodel::addBookmark()
{
    ...
    bookmarkModel->addBookmark("http://wwww.test1.jolla.com", "jolla", "");
...
{
    bookmarkModel->addBookmark("http://wwww.test2.jolla.com", "jolla", "");
    int count = bookmarkModel->rowCount();

    QSignalSpy countChangeSpy(bookmarkModel, SIGNAL(countChanged()));
    bookmarkModel->removeBookmark("http://wwww.test2.jolla.com");
    waitSignals(countChangeSpy, 1);

    QCOMPARE(bookmarkModel->rowCount(), count-1);

    bookmarkModel->removeBookmark("http://wwww.test3.that.is.not.there.jolla.com");
    QCOMPARE(bookmarkModel->rowCount(), count-1);
}
...
{
    bookmarkModel->addBookmark("http://wwww.test4.contains.jolla.com", "jolla", "");

    QVERIFY(bookmarkModel->contains("http://wwww.test4.contains.jolla.com"));
    QVERIFY(!bookmarkModel->contains("http://wwww.test4.that.is.not.there.jolla.com"));
}
